...
function createGameCards(flashcards: Flashcard[]): GameCard[] {
  const selectedCards = flashcards.slice(0, 8)
...
export const MatchGame = ({ flashcards, onComplete }: MatchGameProps) => {
  const [cards, setCards] = useState<GameCard[]>(() =>
    flashcards && flashcards.length >= 2 ? createGameCards(flashcards) : []
  )
  ...
  // Debug logging
  useEffect(() => {
    console.log('[MatchGame] Received flashcards:', flashcards?.length || 0)
  }, [flashcards])

  const totalPairs = Math.min(flashcards?.length || 0, 8)
  ...
  useEffect(() => {
    ...
    const isMatch = firstCard.flashcardId === secondCard.flashcardId
  ...
  const handleRestart = () => {
    setCards(createGameCards(flashcards))
    setSelectedCards([])
    setIsChecking(false)
    setTimer(0)
    setIsComplete(false)
    ...
    return (
      <div className="flex items-center justify-center h-80">
        <p className="text-muted-foreground">
          Need at least 2 flashcards to play the match game.
        </p>
      </div>
    )
  }
    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center justify-center py-12 gap-6">
          ...
          <Button onClick={handleRestart} size="lg" className="min-w-[200px]">
    ...
    <div className="flex flex-col items-center gap-6 py-8">
      ...
      {lastMatchWasCorrect !== null && (
        <div className={cn(
          ...
          lastMatchWasCorrect ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
        )}>
      ...
      <div className="grid grid-cols-4 gap-3 w-full max-w-3xl">
        {cards.map(card => (
          <div
            key={card.id}
            className={cn(
              "match-card aspect-[3/4] cursor-pointer",
              card.isMatched && "flipped",
              card.isFlipped && "flipped"
            )}
            onClick={() => handleCardClick(card.id)}
          >
            <div className="match-card-inner relative w-full h-full">
              {/* Card back (hidden) */}
              <div className={cn(
